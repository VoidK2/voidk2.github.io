...
Spmatrix *cmpSpmatrix(Spmatrix *A,Spmatrix *B,Spmatrix *C){  //未赋值的地址为0<br>
	int ct=0;
	for(int i=0;i<A->t;i++){
		if( (A->data[i].i!=B->data[i].i) || (A->data[i].j!=B->data[i].j) ){    //行或列不相等 不重复<br>
		...
		if( (A->data[i].i==B->data[i].i) && (A->data[i].j==B->data[i].j) ){   //行和列相等<br>
